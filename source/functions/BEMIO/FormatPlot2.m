function FormatPlot2(fig,heading,subtitle,x_lables,y_lables,X_data,Y_data,X1_data,Y1_data,legends,notes)
...
%     axes('Position',[0.0731 0.3645 0.2521 0.4720]);
%     hold('on');
%     box('on');
    plot(X_data,squeeze(Y_data(1,i,:)),'LineWidth',1); 
    hold on
    plot(X1_data,squeeze(Y1_data(1,i,:)),'LineWidth',1) 
    legend(legends(1,:))
    title(subtitle(1));
...
%     subplot(1,3,2)
    ...
    hold on
    plot(X1_data,squeeze(Y1_data(2,i,:)),'LineWidth',1);  %,'Parent',axes2);
    legend(legends(2,:))
    title(subtitle(2));
...
%     subplot(1,3,3)
    ...
    hold on
    plot(X1_data,squeeze(Y1_data(3,i,:)),'LineWidth',1);  %,'Parent',axes3);    
    legend(legends(3,:))
    title(subtitle(3));
...
% legend(axes2,legen    ds(2,:),'location','best','Box','off','Interpreter','none')
