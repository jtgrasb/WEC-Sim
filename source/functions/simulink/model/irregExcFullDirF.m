function [Fext, relYawLast, coeffsLastMD,coeffsLastRE,coeffsLastIM] = irregExcFullDirF(pYaw, A, w, dofGRD,dirGRD,wGRD,qDofGRD,qWGRD,fEHRE,fEHIM, fEHMD, phaseRand,dw,time, directions, Disp, intThresh, prevYaw, prevCoeffMD, prevCoeffRE, prevCoeffIM)
...
coeffsLastMD=zeros(length(directions),length(w),size(dofGRD,1)); %  directions, freq, dof
coeffsLastRE=zeros(length(directions),length(w),size(dofGRD,1));
coeffsLastIM=zeros(length(directions),length(w),size(dofGRD,1));
...
elseif min(relYaw,[],'all')<min(dirGRD,[],'all') 
 ...
 relYawGRD = reshape(repmat(relYaw', size(dofGRD,1), length(w)), size(dofGRD,1), length(directions), length(w));
...
C0 = bsxfun(@times,A,dw);
C1 = sqrt(bsxfun(@times,A,dw));
